...
exports.checkAccess = async (req, res, next) => {
    ...
    try {
        ...
        const [acceptedProposal] = await db.query('SELECT developer_id FROM Proposals WHERE project_id = ? AND status = "Accepted"', [projectId]);

        const isStudentOwner = project[0] && project[0].student_id === userId;
        const isAcceptedDeveloper = acceptedProposal.length > 0 && acceptedProposal[0].developer_id === userId;

        if (isStudentOwner || isAcceptedDeveloper) {
